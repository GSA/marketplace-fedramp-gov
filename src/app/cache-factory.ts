...
export class CacheFactory {
  ...
  isJsonNeeded(): boolean {
    if (
      this.cacheDate != this.formatDate ||
      this.cacheDate == null ||
      this.cacheDate == '' ||
      this.cacheJson == '' ||
      this.cacheJson == null
  ...
  populateCache() {
    ...
    this.cacheJson = LZString.decompressFromUTF16(
      localStorage.getItem('cacheJson') || '{}'
    );
  }
  ...
  reformatSomeDates(inField: string) {
    return inField.indexOf('T') == -1
      ? inField
      : inField.slice(5, 7) + '/' + inField.slice(8, 10);
  }
  ...
  getDateTimeField(inField: string) {
    return inField.indexOf('T') == -1
      ? inField
      : inField.slice(0, inField.indexOf('T'));
  }
